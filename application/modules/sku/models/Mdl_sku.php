...
class Mdl_sku extends MY_Model {

	private $p_key = 'sku_id';
	private $table = 'sku';
	private $alias = 's';
	private $fillable = ['brand_id','sku'];
    private $column_list = ['Brand Name', 'SKU','Date'];
    private $csv_columns = ['Brand Name', 'SKU'];
	...
	function get_collection( $count = FALSE, $sfilters = [], $rfilters = [], $limit = 0, $offset = 0, ...$params ) {
        ...
        $q = $this->db->select('b.brand_id, b.brand_name, s.sku_id, s.sku, s.insert_dt, s.update_dt')
		->from('sku s')
		...
		if(is_array($rfilters) && count($rfilters) ) {
            ...
            foreach($rfilters as $key=> $value) {
                ...
                if($key == 'from_date' && !empty($value)) {
                    $this->db->where('DATE(s.insert_dt) >=', date('Y-m-d', strtotime($value)));
                    continue;
                }
