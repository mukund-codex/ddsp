...
class Mdl_about extends MY_Model {

	private $p_key = 'about_id';
	private $table = 'about';
	private $alias = 'a';
	private $fillable = ['about'];
    private $column_list = ['About'];
    private $csv_columns = ['About'];
		...
		if(is_array($rfilters) && count($rfilters) ) {
            ...
            foreach($rfilters as $key=> $value) {
                ...
                if($key == 'from_date' && !empty($value)) {
                    $this->db->where('DATE(a.insert_dt) >=', date('Y-m-d', strtotime($value)));
                    continue;
                }
	...
	function save(){
        ...
        $response['message'] = 'Congratulations! about has been added successfully.';
        $response['redirectTo'] = 'about/lists';
	...
	function modify(){
		...
		$response['status'] = TRUE;
        $response['message'] = 'Congratulations! record was updatea.';
